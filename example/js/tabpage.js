(function (window) {
    var provider = {
        ...
        setValue: function (target, apName, value, options) {
            if (!target.knotExampleTabPage) {
                ...
                window.Knot.Advanced.setDataContext(tabPageNode, model);
    ...
    window.Knot.Advanced.registerNamedGlobalSymbol("createKnotExampleTabPageContent", function (page){
        var node =  window.Knot.Advanced.createFromTemplate(page.template, page.data, this);
        window.Knot.Advanced.setDataContext(node, page.data);
        return node;
    });
