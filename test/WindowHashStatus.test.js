...
//    });
    ...
    global.QUnit.asyncTest("private.WindowHashStatus.test hash", function (assert) {
        ...
        var testFunc = function(){
            assert.equal(scope.Utility.getValueOnPath(null, "$hash.originalHash"), "test", "test hash is correctly set");
            scope.DataObserver.stopMonitoring(scope.Utility.getValueOnPath(null, "$hash"), "originalHash", testFunc);
            window.location.hash = "";
            global.QUnit.start();
        };
        scope.DataObserver.monitor(scope.Utility.getValueOnPath(null, "$hash"), "originalHash", testFunc);
        window.location.hash = "#test";
    });
    ...
    global.QUnit.test("private.WindowHashStatus.test set fields", function (assert) {
        ...
        assert.equal(scope.Utility.getValueOnPath(null, "$hash.originalHash"), "c2/t1/123",  "hash is changed");
        ...
        assert.equal(scope.Utility.getValueOnPath(null, "$hash.originalHash"), "c2/t2/678",  "hash is changed");
        ...
        assert.equal(scope.Utility.getValueOnPath(null, "$hash.originalHash"), "c2",  "hash is changed");

        scope.Utility.setValueOnPath(null, "$hash.type", null);
        scope.Utility.setValueOnPath(null, "$hash.id", 123);
        assert.equal(window.location.hash, "#c2//123", "hash is changed");
        assert.equal(scope.Utility.getValueOnPath(null, "$hash.originalHash"), "c2//123",  "hash is changed");
        window.location.hash = "";
    });
})(window);